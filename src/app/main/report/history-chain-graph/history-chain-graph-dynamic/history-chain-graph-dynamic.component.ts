...
export class HistoryChainGraphDynamicComponent {
  ...
  public diagramModelChange = function (this: HistoryChainGraphDynamicComponent, changes: go.IncrementalData) {
    if (!changes) return;
    const appComp: any = this;
    this.state = produce(this.state, (draft: { skipsDiagramUpdate: boolean; diagramNodeData: go.ObjectData[]; diagramLinkData: go.ObjectData[]; diagramModelData: go.ObjectData; selectedNodeData: go.ObjectData; }) => {
  ...
  public ngAfterViewInit() {
    ...
    const appComp: HistoryChainGraphDynamicComponent = this;
    ...
    this.myDiagramComponent.diagram.addDiagramListener(
      ...
      function (e) {
        ...
        appComp.state = produce(appComp.state, (draft: { diagramNodeData: any[]; selectedNodeData: null; }) => {
          if (node instanceof go.Node) {
            var idx = draft.diagramNodeData.findIndex(
              (nd: { key: any; }) => nd.key == node.data.key
            );
  ...
  public handleInspectorChange(changedPropAndVal: { prop: any; newVal: any; }) {
    ...
    this.state = produce(this.state, (draft: { selectedNodeData: any; diagramNodeData: any[]; skipsDiagramUpdate: boolean; }) => {
      var data = draft.selectedNodeData;
      data[path] = value;
      const key = data.key;
      const idx = draft.diagramNodeData.findIndex((nd: { key: any; }) => nd.key == key);
      if (idx >= 0) {
